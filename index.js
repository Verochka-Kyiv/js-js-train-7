...
function createAndCompareTwoSymbols(value) {
  ...
  let syb1 = Symbol(value);
  let syb2 = Symbol(value);
  return syb1 === syb2;
}
...
let myObject = {
  ...
  next() {
    ...
    if (this.currentIndex < this.category.length) {
      ...
      // Повертаємо об'єкт з властивістю value значенням якої буде value,
      // та прапорцем done: false
      return { value, done: false };
    }

    // Якщо властивість об'єкту currentIndex більше або дорівнює довжині масиву category,
    // повертаємо об'єкт з прапорцем done: true, коли ітерація закінчена
    return { done: true };
  },
};
